...
begin
	select @signatory_name = isnull(@signatory_name, first_name + ' ' + last_name), @signatory_personal_tax_num = isnull(@signatory_personal_tax_num, personal_tax_num)
	from contact c
...
begin
	...
	declare @contract_id int, @description varchar(100) = '', @bank_contract_status int
	...
	begin
        ...
        begin
        	...
        	begin catch
				set @error += isnull(@error, case when @error is null then 'cds_UpdateBank threw an exception for bank_contract_id: ' + @bank_contract_id  else '' end)
			end catch
