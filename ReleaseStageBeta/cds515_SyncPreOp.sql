...
/*						update o set ord_contact_type_id = b.ord_contact_type_id
				...
				if @table_name = 'order_account'
					begin
						...
						,@WL_ord_contact_type_id	int = (select ord_contact_type_id from cds.dbo.ord_contact_type where code = 'WL')
						...
						while @min_id <= @max_id
							begin
								begin try
									select  @Error = null, @cds_515_order_account_id = null, @contract_id = null, @cds_ord_cust_id = null, @email = null, @ord_contact_id = null
									...
									if not exists (select 1 from cds.dbo.contract where contract_id = @contract_id and end_date is null )
										begin
											...
											select @email = coalesce(nullif(c.email1,''), nullif(c.email2,'')), @ord_contact_id = ord_contact_id
											from cds.dbo.ord_contact c
											join cds.dbo.ord_cust oc on c.ord_cust_id = oc.ord_cust_id
											where oc.ord_cust_id = @cds_ord_cust_id
											and c.ord_contact_type_id = @WL_ord_contact_type_id
											and coalesce(nullif(c.email1,''), nullif(c.email2,'')) is not null
...
--													;throw 50517, @Error, 1
											...
											if nullif(@Error,'') is null
											 begin
													...
													,@bank_account_name = a.bank_account_name, @ord_contact_id = c.ord_contact_id
													...
													execute cds.dbo.cds_InsertContract @product_id	 = @product_id, @start_date = @start_date, @email = @email, @bank_account = @bank_account, @account_num = @account_num, @facility_id = @facility_id, @commodity_id = @commodity_id, @personal_tax_num = @personal_tax_num, @business_tax_num = @business_tax_num,@Process = @Process
														,@contract_id = @contract_id output, @Error = @Error output, @ord_contact_id = @ord_contact_id
