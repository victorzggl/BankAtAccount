...
cross apply dbo.cds_fn_GetActiveBankForAccount (isnull(a.account_id,a2.account_id), oc.cust_id, 1, oa.bank_account) gabfa
...
while @i <= @total
	...
	set @signatory_personal_tax_num = isnull(nullif(@signatory_personal_tax_num,''),'FAKE TAX NUM' ) --*** TEMPORARY HACK UI does not not capture this data.
	...
	--@OrdAccount_account_id and green_upgrade_flag shouldn't be needed since they shouldn't be null account_id also but is safety check
		begin
			...
			if @contract_id is not null and @DataErrorFlag = 0
				begin
